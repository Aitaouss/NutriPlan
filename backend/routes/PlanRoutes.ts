...
export default async function PlansRoutes(server: any) {
  ...
  server.post(
    "/plans",
    async (request: PlanRequest, reply: FastifyReply) => {
      try {
        if (!request.body) {
        ...
        if (!profile) {
          return reply.status(404).send({ error: "User profile not found. Please complete onboarding first." });
        }
  ...
  server.get("/plans", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { user_id, latest } = request.query as { user_id?: string; latest?: string };
      ...
      if (latest === 'true') {
        ...
        query = "SELECT * FROM plans WHERE user_id = ? ORDER BY created_at DESC LIMIT 1";
        params = [user_id];
      } else {
        // Get all plans for the user
        query = "SELECT * FROM plans WHERE user_id = ? ORDER BY created_at DESC";
        params = [user_id];
      }

      const plans = latest === 'true' 
        ? await server.db.get(query, params)
        : await server.db.all(query, params);

      if (!plans || (Array.isArray(plans) && plans.length === 0)) {
        return reply.status(404).send({ error: "No plans found for this user" });
      }
      ...
      return reply.send({
        message: latest === 'true' ? "Latest plan retrieved" : "Plans retrieved",
        plans: parsedPlans
      });
  ...
  server.put(
    ...
    async (request: UpdatePlanRequest, reply: FastifyReply) => {
      try {
        ...
        if (!plan_data && !bmi) {
          return reply.status(400).send({ error: "plan_data or bmi is required for update" });
        }
